// Package main — точка входа projects-service.
//
// @title           Projects Service API
// @version         1.0
// @description     Управление проектами, задачами и комментариями.
// @BasePath        /
...
import (
	...
	_ "github.com/student-pm/projects-service/docs"

	"github.com/student-pm/projects-service/internal/config"
	...
	"github.com/student-pm/projects-service/internal/pkg/logger"
	"github.com/student-pm/projects-service/internal/pkg/validator"
	"github.com/student-pm/projects-service/internal/repository"
...
func run() error {
	...
	log.Info().Str("env", cfg.App.Env).Msg("starting projects-service")
	...
	repos := repository.NewRepos(pool)
	verifier, err := jwtpkg.New(cfg.JWT.Secret)
	if err != nil {
		return fmt.Errorf("jwt verifier: %w", err)
	}
	svc := usecase.NewService(repos.Projects, repos.Tasks, repos.Comments, time.Now)
	v := validator.New()
